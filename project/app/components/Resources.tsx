...
export default function ResourcesSection() {
  ...
  const resources = [
    {
      icon: <BookOpen className="w-8 h-8 text-[#752F8A] mb-4" />,
      title: "Learning Materials",
      items: ["Technical Workshops", "Tutorial Videos", "Code Repositories", "Study Materials"],
    },
    {
      icon: <Users className="w-8 h-8 text-[#752F8A] mb-4" />,
    ...
    >
      <div className="w-[calc(100%-4rem)] mx-auto rounded-md overflow-hidden">
        <div className="bg-black backdrop-blur-sm p-8">
          <h2 className="text-3xl font-bold mb-8 text-[#752F8A]">Resources</h2>
                ...
                <ul className="space-y-2 text-gray-400">
                  {resource.items.map((item, i) => (
                    <li key={i} className="flex items-center space-x-2">
                      <span className="text-[#752F8A]">•</span>
                      <span className="group-hover:text-[#752F8A] transition-colors duration-300">{item}</span>
                    </li>
                  ))}
                </ul>
              </motion.div>
